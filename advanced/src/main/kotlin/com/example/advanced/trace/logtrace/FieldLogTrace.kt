...
class FieldLogTrace : LogTrace {
    ...
    override fun begin(message: String?): TraceStatus {
        ...
        HelloTraceV2.log.info("[${traceId.id}] ${content}${message}")
    ...
    private fun complete(status: TraceStatus, e: Exception?) {
        ...
        if (e == null) {
            ...
            HelloTraceV2.log.info("[${traceId.id}] ${content}${status.message} time=${resultTimeMs}ms")
        } else {
            val content = addSpace(EX_PREFIX, traceId.level)
            HelloTraceV2.log.info("[${traceId.id}] ${content}${status.message} time=${resultTimeMs}ms ex=${e.toString()}")
        }
